...
# ── Stage 2: DocumentAnalyzer — document identity ──────────────────────────
...
class DocMeta(BaseModel):
    ...
    document_type: DocumentType = Field(
        default="general",
        description="Coarse document form, e.g. report | contract | manual.",
    )
    document_date: str = Field(
        default="",
        description="Primary document date as ISO-8601 date when one can be inferred.",
    )
    document_date_ts: int = Field(
        default=0,
        description="Unix epoch seconds of document_date — used for semantic date filters.",
    )
    period_start: str = Field(
        default="",
        description="Start of the document's covered period as ISO-8601 date.",
    )
    period_start_ts: int = Field(
        default=0,
        description="Unix epoch seconds of period_start.",
    )
    period_end: str = Field(
        default="",
        description="End of the document's covered period as ISO-8601 date.",
    )
    period_end_ts: int = Field(
        default=0,
        description="Unix epoch seconds of period_end.",
    )
    ...
    indexed_at: str = Field(
        default="", description="ISO-8601 UTC timestamp of ingestion."
    )
    indexed_at_ts: int = Field(
        default=0, description="Unix epoch seconds of ingestion time."
    )
    language: LanguageCode = Field(
        default="unknown",
        description="ISO 639-1 language code from the supported top-language set.",
    )
    audience: AudienceType = Field(
        default="general",
        description=(
            "Primary intended audience: technical | management | legal | "
            "financial | scientific | operational | public | general."
        ),
    )
    page_start: int = Field(
        default=0,
        description="1-based first source page covered by this document or chunk.",
    )
    page_end: int = Field(
        default=0,
        description="1-based last source page covered by this document or chunk.",
    )
    page_count: int = Field(
        default=0,
        description="Total number of source pages in the original document.",
    )


# ── Stage 2: DocumentAnalyzer — embedding provenance ───────────────────────
...
# Heading context is extracted from chunk content by ChunkAnnotator instead.


# ── Stage 5: ChunkAnnotator ─────────────────────────────────────────────────
...
class ChunkStructureMeta(BaseModel):
    ...
    chunk_type: ChunkType = Field(
        default="text",
        description=(
            "Dominant content type: text | code | table | list | figure_caption | "
            "hier_summary_section | hier_summary_doc."
        ),
    )


# ── Stage 6: ChunkAnalyzer ───────────────────────────────────────────────
...
class Entities(BaseModel):
    ...
    (chunk_analyzer.py).  Stored in Qdrant as flat ``ent_*`` fields on
...
class SemanticMeta(BaseModel):
    """AI-generated contextual and semantic metadata produced by ChunkAnalyzer."""
    ...
    classification: ChunkClassification = Field(
        default="other",
        description="Chunk classification label from the configured taxonomy.",
    )
